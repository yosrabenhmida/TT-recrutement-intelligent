...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await api.post("/candidate-auth/login", {
        email: email.trim().toLowerCase(),
        password,
      });

      console.log("LOGIN CANDIDAT RÉUSSI :", data);

      // Sauvegarder le candidat dans AuthContext
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#03214f] font-[Inter,system-ui,sans-serif]">
      {/* fond mesh */}
      <div className="absolute inset-0 -z-10 opacity-90 bg-[radial-gradient(60%_60%_at_15%_18%,#0b4e9b_0%,transparent_60%),radial-gradient(50%_50%_at_85%_10%,#29abe2_0%,transparent_60%),radial-gradient(55%_55%_at_75%_85%,#92278f_0%,transparent_60%),radial-gradient(45%_45%_at_25%_90%,#00a99d_0%,transparent_60%)]" />
      <div className="absolute inset-0 -z-10 opacity-[0.07] [background-image:linear-gradient(#fff_1px,transparent_1px),linear-gradient(90deg,#fff_1px,transparent_1px)] [background-size:56px_56px] [mask-image:radial-gradient(70%_60%_at_50%_40%,#000,transparent)]" />
        ...
        <div className="w-full max-w-md">
          ...
          <div className="rounded-[32px] border border-white/20 tt-glass-dark p-8 shadow-[0_40px_100px_-30px_rgba(0,0,0,.6)] sm:p-10">
            <div className="mb-8 text-center">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/25 tt-glass-dark px-4 py-1.5 text-[11px] font-bold uppercase tracking-[.16em] text-white/90">
                Espace Candidat
              </span>
              <h1 className="mt-5 text-2xl font-black text-white sm:text-3xl">
                Bon retour <span className="tt-gradient-text">chez TT</span>
              </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <label className="group relative block">
                ...
                <input
                  ...
                  required
                  className="w-full rounded-2xl border border-white/15 bg-white/5 py-4 pl-12 pr-4 text-sm font-medium text-white placeholder:text-white/40 outline-none transition focus:border-[#29ABE2] focus:bg-white/10 focus:ring-4 focus:ring-[#29ABE2]/15"
                />
              </label>
              ...
              <button
                type="submit"
                disabled={loading}
                className="group relative mt-2 flex w-full items-center justify-center gap-2 overflow-hidden rounded-full bg-white px-8 py-4 text-sm font-black text-[#0B4E9B] shadow-2xl transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-60 disabled:hover:scale-100"
              >
          ...
          <div className="mt-6 border-t border-white/10 pt-5 text-center">
            <p className="text-xs text-blue-100/50">
              Vous êtes administrateur / RH ?{" "}
              <Link
                to="/admin/login"
                className="font-bold text-[#29ABE2] transition hover:text-white"
              >
